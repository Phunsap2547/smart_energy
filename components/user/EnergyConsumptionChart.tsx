...
export default function EnergyConsumptionChart() {
  ...
  // หาค่าต่ำสุดแบบ Dynamic
  const lowestPowerTime = useMemo(() => {
    if (!energyConsumptionSeries || energyConsumptionSeries.length === 0) return null;
    return energyConsumptionSeries.reduce((minItem, currentItem) => {
      return currentItem.power < minItem.power ? currentItem : minItem;
    }, energyConsumptionSeries[0]).time;
  }, []);
        ...
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={energyConsumptionSeries} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line
              ...
              strokeWidth={2}
              dot={(props) => {
                const isLowest = props.payload.time === lowestPowerTime;
                return (
                  <Dot
                    key={props.index}
                    cx={props.cx}
